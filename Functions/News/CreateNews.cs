    ...
    public static class CreateNews
    {
            ...
            ILogger log)
        {
            ...
            if (opration == "delete")
            {
                try
                {
                    Database.DeleteNews(int.Parse(title));
                    return new OkObjectResult(new BaseResponse(token, "News Deleted Successfully"));
                }
                catch (Exception e)
                {
            ...
            Database.CreateNews(FunctionUtility.HTMLEncoder(title), FunctionUtility.HTMLEncoder(description), link, category, imagePath, creatorAdminId);
            SendNotification.SendNotifications(title, description, "all");
