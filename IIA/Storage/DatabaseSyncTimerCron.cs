...
namespace IIABackend
{
    ...
    /// </summary>
    public static class DatabaseSyncTimerCron
    {
        /// <summary>
        /// Function
        /// </summary>
        ...
        /// <returns>Success of Database Sync</returns>
        [FunctionName("DatabaseSyncTimerCron")]
        public static async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Function, "post", Route = null)] HttpRequest req, ILogger log)
        {
            log.LogInformation("DatabaseSync Cron Started");
            ...
            try
            {
                ...
                /*check whether oldprofile is accepting dob and dom or not ang change userprofile as such*/
                ...
                int counts = 0;
                Dictionary<string, int> newData = Database.GetNewMember();
                foreach (dynamic oldProfile in list)
                {
                    try
                    {
                            if (!newData.ContainsKey(oldProfile.MemberId) || newData[oldProfile.MemberId] < DateTime.ParseExact(oldProfile.Expirydate, "dd/MM/yyyy", null).Year)
                            {
                                counts++;
                                UserProfile userProfile = new UserProfile(int.MinValue, oldProfile.Mobile.Substring(0, 10), oldProfile.MemberId, int.Parse(oldProfile.Admissionfee.Contains(".") ? oldProfile.Admissionfee.Substring(0, oldProfile.Admissionfee.IndexOf(".")) : oldProfile.Admissionfee), int.Parse(oldProfile.Annualsubscription.Contains(".") ? oldProfile.Annualsubscription.Substring(0, oldProfile.Annualsubscription.IndexOf(".")) : oldProfile.Annualsubscription), DateTime.ParseExact(oldProfile.Expirydate, "dd/MM/yyyy", null).Year, DateTime.ParseExact(oldProfile.MemberJoinDate, "dd/MM/yyyy", null), DateTime.ParseExact(oldProfile.Lastrenewaldate, "dd/MM/yyyy", null), DateTime.ParseExact(oldProfile.Expirydate, "dd/MM/yyyy", null).Year.ToString(), GetprofileStatus(DateTime.ParseExact(oldProfile.Expirydate, "dd/MM/yyyy", null)), int.Parse(oldProfile.ChapterId), oldProfile.ChapterName, oldProfile.UnitName, oldProfile.GSTIN, string.Empty, oldProfile.IndustryStatus, oldProfile.Address, oldProfile.District, oldProfile.City, oldProfile.State, "India", oldProfile.Pincode, oldProfile.WebsiteUrl, oldProfile.ProductCategory, oldProfile.ProductSubCategory, oldProfile.MajorProducts, oldProfile.AnnualTurnOver, oldProfile.EnterpriseType, oldProfile.Exporter, oldProfile.Classification, oldProfile.Owner.Trim().Contains(" ") ? oldProfile.Owner.Trim().Split(" ")[0] : oldProfile.Owner.Trim(), oldProfile.Owner.Trim().Contains(" ") ? oldProfile.Owner.Trim().Substring(oldProfile.Owner.Trim().IndexOf(" ")) : string.Empty, oldProfile.Email, oldProfile.DateOfBirth, oldProfile.DateOfMarriage, string.Empty, string.Empty, string.Empty, -2, -2, DateTime.Now, DateTime.Now);
                                int memberJoinYear = DateTime.ParseExact(oldProfile.MemberJoinDate, "dd/MM/yyyy", null).Year;
                                int memberExpiryYear = DateTime.ParseExact(oldProfile.Expirydate, "dd/MM/yyyy", null).Year;
                                ...
                                Database.DatabaseSyncInsert(userProfile, -2);
                            }
                        }
                    catch (Exception)
                    {
                ...
                res.Add("count", counts);
                return new OkObjectResult(res);
            }
            catch (Exception e)
            {
                log.LogInformation(e.Message);
                return new OkObjectResult(new BaseResponse(null, e.Message));
            }
